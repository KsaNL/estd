...
#	define eoutf(func) \
	__declspec(deprecated("Using " # func " instead..."))
...
wchar_t* _us2wcs(
	...
	_Out_ size_t * opt = 0) {
	unsigned value, num, u;
	wchar_t* dst = (wchar_t*)ealloc(strlen(ustr) << 1), * bak = dst;
	...
	while (u = (*ustr & 0xFF)) {
		if (u < 0x80) {
			*dst++ = (wchar_t)u;
			...
			if ((u & 0xF0) == 0xE0) { value = u & 0x0f; num = 3; }
			else if ((u & 0xE0) == 0xC0) { value = u & 0x1f; num = 2; }
			else if ((u & 0xF8) == 0xF0) { value = u & 0x07; num = 4; }
			else { num = 0; value = 0; }
...
char* _wcs2us(
	...
	_Out_ size_t * opt = 0) {
	char* dst = ealloc((wcslen(wbuf) << 1) | sizeof(int)), * bak = dst;
	if(!dst) return nullptr;
	while (*wbuf) {
		unsigned w = *wbuf++;
		if (w < 0x80)
			...
			*dst++ = 0xe0 | ((w & 0xF000) >> 12);
			*dst++ = 0x80 | ((w & 0xFC0) >> 6);
			*dst++ = 0x80 | ((w & 0x3f));
		}
		else {	//(w <= 0x10ffff)		
			*dst++ = 0xF0 | ((w & 0x1C0000) >> 18);
			*dst++ = 0x80 | ((w & 0x3F000) >> 12);
			*dst++ = 0x80 | ((w & 0xFC0) >> 6);
			*dst++ = 0x80 | ((w & 0x3F));
		}
	}
...
public:
	...
	estr(char* fmt) : asize(0) {
		this->assign(fmt, ~0); // asdata need nullptr
	}
	...
	void append(const char* src, size_t usr = ~0) {
		...
		if (char* dat = ealloc(this->asize + usr + 4)) {
	...
	char* find(const char* buf) {
		return strstr(this->asdata, buf);
	}
	bool contain(int _c) {
		return this->asize ? !!strchr(this->asdata, _c) : false;
	}
	//
	...
	//
	bool operator==(const char* str) { return esame==strcmp(str, this->asdata); }
	template <size_t N> // cmp this left part
	bool operator>=(const char(&str)[N]) { return esame==memcmp(str, this->asdata, N - 1); }
	template <size_t N> // cmp obj left part
	bool operator<=(const char(&obj)[N]) { return esame==memcmp(this->asdata,obj, this->asize); }
	//
	template <size_t N>
	...
	operator bool  () { return this->asize !=0; }
	...
	~estr() { if (asdata) ::free(asdata); }
private:
	char* asdata;
	size_t asize;

	bool _is_fmt(const char* var) { 
		this->asize=0; while (*var) { if (*var++=='%')return true;asize++; }
		return false;
	}
...
public:
	...
	bool parse(const char* j_str, size_t j_size = ~0, cstr jstr = 0) {
		if (this->isfather()) {
			if ((jstr = zip_json(j_str, j_size))) {
				this->core_json(this, jstr);
				this->aname = "Obj";
				this->asori = jstr;
				this->asuh = 0;
				return true;
			}
			this->asori = (char*)"null";
			this->asuh = "null"_us;
			this->asttr = ej_null;
			this->amap = ~0;
		}
	...
	template<size_t N>
	ejson& operator[] (const char(&name)[N]) {
	...
	operator bool() { return !!this->amap; }
	operator char* () { return this->asori ? this->asori : (char*)""; }
	#ifdef _WIN64
...
private:
	...
	inline bool isfather() { return asuh == 0; }
	...
	static 
	void core_json(ejson* js_, cstr p) {
		ejson &js = *js_, * jss; cstr s;
		uint u = calc_json(p); dbi(u == ~0);
		if (u > 0 && u != ~0) {
			jss = new ejson[u];
			js.amap = (iint)jss;
			js.asttr = ej_json | (u & ej_size);

			if (*p == '{') p++;
			while (*p++ == '"') {
				s = strchr(p, '":'); dbi(!s, s, p); if(s)*(s++ - 1) = '\0';
				p = parse_json(jss++, s, p);
				if (*p == '}' || *p == ',') p++;
			}
		}
	}
	...
	static 
	void coreary_json(ejson* jss, cstr p) {
		ejson& js = *jss; cstr s;
	...
	char* parse_json(ejson* jss, cstr v, cstr n, uint uh = 0) {
		...
		switch (*v) {
			...
			case '5':case '6':case '7':case '8':case '9':
				n = strchr(v, ',');if (!n)n =v+strlen(v)-1;else *n='\0'; dbi(!n, v);
			...
			case '"':case 'n':case 'N': case 't':case 'T':case 'f':case 'F':
				...
				if (*v == '"') {
					...
					js.asttr = ej_string | (ej_size & (n-1-v)); 
				}
				else {
					js.asori = v; js.amap = iint(v);
					if (*v == 't' || *v == 'T') {
			...
			case '{': {
				n = next_json(v); dbi(!n, "not found"); if (n)*n++ = '\0';
				core_json(jss, v);
				return n;
			}
